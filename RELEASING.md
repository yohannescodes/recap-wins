...
A release produces **two artifacts from one prose source**:

- [`docs/changelogs/v<version>.html`](docs/changelogs/) — the rendered
  release page, linked from the GitHub Release body. Self-contained,
  PG-blog aesthetic, "Generated by rw" credit.
- [`CHANGELOG.md`](CHANGELOG.md) — the cumulative scrollable index
  (Keep-a-Changelog format), with a new section prepended at the top
  linking to the HTML page. This is the file people read on the repo
  landing page.

The HTML page is what `rw notes --changelog` writes directly. The
`scripts/render-changelog.sh` wrapper produces **both** — it renders the
HTML through the live `HTMLRender.changelog`, then prepends a matching
section to `CHANGELOG.md` (demoting `##` headings to `###` so the version
itself is the section root). Idempotent: re-running on the same version
regenerates the HTML but leaves the markdown alone.

Two paths, depending on whether you have an API key handy:

**API mode** — `rw notes --changelog` writes the HTML directly. For the
matching `CHANGELOG.md` section, pipe the prose through the script:

```sh
# HTML page only (just the rendered release page):
...
wraps it in the HTML page through the same renderer the binary uses AND
prepends a matching section to `CHANGELOG.md`:
...
# 3. Wrap the prose in the canonical HTML page AND prepend to CHANGELOG.md.
scripts/render-changelog.sh <version> < /tmp/changelog.md
# writes docs/changelogs/v<version>.html
# prepends a new section to CHANGELOG.md

git add docs/changelogs/v<version>.html CHANGELOG.md
```
